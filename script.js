    ...
    if (!incompatible_already_grouped) {
      ...
      new_incompatible_namespaces.forEach((new_incompatible_namespace) => {
        const new_incompatible_already_grouped =
          incompatible_namespaces.includes(new_incompatible_namespace);
        const new_incompatible_in_queue =
          incompatible_namespaces_queue.includes(new_incompatible_namespace);
        const push_new_incompatible =
          !new_incompatible_already_grouped && !new_incompatible_in_queue;
...
function buildEnchantList(item_namespace_chosen) {
  ...
  function filterEnchantmentGroup(enchantment_namespace) {
    const is_valid_enchantment = item_enchantment_namespaces.includes(
  ...
  item_enchantment_namespaces.forEach((enchantment_namespace) => {
    ...
    var enchantment_group = incompatibleGroupFromNamespace(
      enchantment_namespace
    );
...
function displayInstructionText(instruction) {
  ...
  const prior_work_text =
    languageJson.prior_work_penalty + displayLevelsText(prior_work_penalty);
